...
const piani = [
  ...
  },
  {
    ...
    canoneInclusi: [
      ...
      "Gestione profilo Social – Area Wellness (4 pubbl./mese)",
      "Sponsorizzazioni LinkedIn – Area Performance",
      "Sponsorizzazioni Meta – Area Wellness",
    ],
  },
  {
    ...
    canoneInclusi: [
      ...
      "Gestione profilo Social – Area Wellness (8 pubbl./mese)",
      "Sponsorizzazioni LinkedIn – Area Performance",
      "Sponsorizzazioni Meta – Area Wellness",
    ],
  },
    ...
    <section id="pacchetti" className="w-full py-16 md:py-24 px-6 md:px-10 lg:px-20" style={{ background: P.surface }}>
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-3 mb-5">
          ...
          <span style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "9px", color: P.accent, letterSpacing: "0.28em", textTransform: "uppercase" }}>Investimento</span>
        </div>
            ...
            <div key={pp.nome}
              ...
              {pp.highlight && (
                <div className="text-center py-1.5" style={{ background: `${pp.c}15`, borderBottom: `1px solid ${pp.c}20` }}>
                  <span style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "8px", color: pp.c, letterSpacing: "0.2em", textTransform: "uppercase" }}>✦ Più Scelto</span>
                </div>
              )}
              ...
              <div className="p-6 flex flex-col flex-1 gap-5">
                {/* Header */}
                <div>
                  <span style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "7px", color: pp.c, border: `1px solid ${pp.c}28`, background: `${pp.c}07`, borderRadius: "3px", padding: "2px 7px", letterSpacing: "0.15em", textTransform: "uppercase" }}>{pp.focus}</span>
                ...
                <div className="rounded-2xl p-4" style={{ background: pp.golden ? "rgba(255,255,255,0.04)" : P.surfaceAlt, border: `1px solid ${pp.golden ? "rgba(255,255,255,0.06)" : P.border}` }}>
                  <p style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "8px", color: pp.c, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: "4px" }}>UNA TANTUM</p>
                  ...
                  <p style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "7px", color: pp.golden ? `${P.textInv}35` : P.textMuted, letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: "6px" }}>Include</p>
                  <div className="flex flex-col gap-1.5">
                    {pp.unatantumInclusi.map(v => (
                      <div key={v} className="flex items-start gap-2">
                        ...
                        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "10.5px", color: pp.golden ? `${P.textInv}60` : P.textSub, lineHeight: "1.45" }}>{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-2xl p-4" style={{ background: pp.golden ? "rgba(255,255,255,0.03)" : P.bg, border: `1px solid ${pp.golden ? "rgba(255,255,255,0.05)" : P.border}` }}>
                  <p style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "8px", color: pp.c, letterSpacing: "0.2em", textTransform: "uppercase", marginBottom: "4px" }}>CANONE MENSILE</p>
                  <div className="flex items-baseline gap-1 mb-2">
                    ...
                    <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "12px", color: pp.golden ? `${P.textInv}45` : P.textMuted }}>{pp.canoneRaw}</span>
                  </div>
                  <p style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "7px", color: pp.golden ? `${P.textInv}35` : P.textMuted, letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: "6px" }}>Include</p>
                  <div className="flex flex-col gap-1.5">
                    {pp.canoneInclusi.map(v => (
                      <div key={v} className="flex items-start gap-2">
                        ...
                        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: "10.5px", color: pp.golden ? `${P.textInv}55` : P.textSub, lineHeight: "1.45" }}>{v}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-1" style={{ borderTop: `1px solid ${pp.golden ? "rgba(255,255,255,0.06)" : P.border}` }}>
                  <span style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "8px", color: pp.golden ? `${P.textInv}35` : P.textMuted, letterSpacing: "0.12em" }}>Totale {pp.totaleMesi}</span>
                ...
                <a href="mailto:support@denani.it"
                  className="rounded-xl py-3 flex items-center justify-center transition-all duration-200 hover:opacity-90"
                  style={{
                    background: pp.golden ? pp.c : `${pp.c}12`,
                    border: `1px solid ${pp.c}35`,
                    fontFamily: "'Montserrat', sans-serif",
                    fontSize: "12px", fontWeight: 700,
                    color: pp.golden ? "#FFFFFF" : pp.c,
        ...
        <div className="rounded-2xl p-6 md:p-8" style={{ background: P.bg, border: `1px solid ${P.border}` }}>
          <p style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "9px", color: P.accent, letterSpacing: "0.25em", textTransform: "uppercase", marginBottom: "14px" }}>Note</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {note.map((n, i) => (
              <div key={i} className="flex items-start gap-3">
                <span style={{ fontFamily: "'Roboto Mono', monospace", fontSize: "9px", color: P.accentLight, letterSpacing: "0.1em", flexShrink: 0, marginTop: "2px" }}>
                  {String(i + 1).padStart(2, "0")}
                </span>
                <p style={{ fontFamily: "'Inter', sans-serif", fontSize: "11px", color: P.textSub, lineHeight: "1.65" }}>{n}</p>
              </div>
            ))}
          </div>
        </div>
