  ...
  return (
    <section className="relative w-full overflow-hidden" style={{ background: "#0D1520", paddingBottom: "80px" }}>
      ...
      <div className="px-6 md:px-20 pt-12 pb-4">
        ...
        <div className="flex-1 min-w-0 rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 40px 80px rgba(0,0,0,0.6)" }}>
          ...
          <div style={{ background: "#0B0F14" }}>
            ...
            <div className="relative overflow-hidden" style={{ minHeight: "520px" }}>
              <img src={perfVO2} alt="VO2 Max Test" className="absolute inset-0 w-full h-full object-cover"
                style={{ filter: "brightness(0.68) saturate(0.8) hue-rotate(195deg)" }} />
              {/* Grid overlay */}
        ...
        {/* Phone mockup — Performance mobile */}
        <div className="xl:self-center flex-shrink-0" style={{ width: "224px" }}>
          <div style={{ background: "#1A2235", borderRadius: "40px", border: "2px solid rgba(255,255,255,0.14)", padding: "14px 10px 18px", boxShadow: "0 40px 80px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.07)" }}>
            {/* Dynamic island */}
            <div style={{ width: "76px", height: "24px", background: "#0D1520", borderRadius: "12px", margin: "0 auto 10px", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px" }}>
              <div style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#1A2235" }} />
            </div>
            {/* Screen */}
            <div style={{ borderRadius: "26px", overflow: "hidden", background: "#0B0F14" }}>
              {/* Status bar */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 16px 4px", background: "#0B0F14" }}>
                <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "9px", fontWeight: 700, color: "rgba(238,242,255,0.9)" }}>9:41</span>
                <div style={{ display: "flex", gap: "5px", alignItems: "center" }}>
                  <svg width="11" height="8" viewBox="0 0 12 9" fill="rgba(238,242,255,0.7)"><rect x="0" y="3" width="2" height="6" rx="0.5"/><rect x="3" y="2" width="2" height="7" rx="0.5"/><rect x="6" y="0" width="2" height="9" rx="0.5"/><rect x="9" y="0" width="2" height="9" rx="0.5" opacity="0.3"/></svg>
                  <svg width="15" height="9" viewBox="0 0 24 12"><rect x="0" y="1" width="21" height="10" rx="2.5" stroke="rgba(238,242,255,0.7)" strokeWidth="1.5" fill="none"/><rect x="2" y="3" width="14" height="6" rx="1" fill={`${P.perfBlue}`}/><rect x="22" y="4" width="2" height="4" rx="1" fill="rgba(238,242,255,0.4)"/></svg>
                </div>
              </div>
              {/* Mobile nav */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 14px", background: "rgba(11,15,20,0.98)", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
                  <div style={{ width: "3px", height: "16px", background: P.perfBlue, borderRadius: "2px" }} />
                  <div>
                    <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "10px", fontWeight: 900, color: "#EEF2FF", letterSpacing: "0.04em", margin: 0 }}>COSMED</p>
                    <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "6px", fontWeight: 700, color: P.perfBlue, letterSpacing: "0.1em", textTransform: "uppercase", margin: 0, lineHeight: "1.2" }}>PERFORMANCE</p>
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: "3.5px" }}>
                  {[0,1,2].map(i => <div key={i} style={{ width: "18px", height: "1.5px", background: "rgba(238,242,255,0.55)", borderRadius: "1px" }} />)}
                </div>
              </div>
              {/* Hero */}
              <div style={{ position: "relative", height: "150px", overflow: "hidden" }}>
                <img src={perfVO2} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", filter: "brightness(0.62) saturate(0.75) hue-rotate(195deg)" }} />
                ...
                <div style={{ position: "absolute", inset: 0, padding: "14px 16px", display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
                  ...
                  <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "16px", fontWeight: 900, color: "#EEF2FF", letterSpacing: "-0.04em", lineHeight: "0.9", marginBottom: "12px" }}>MISURA.<br />ANALIZZA.<br /><span style={{ color: P.perfBlue }}>OTTIMIZZA.</span></p>
                  <div style={{ display: "inline-flex", background: P.perfBlue, borderRadius: "6px", padding: "6px 12px", width: "fit-content" }}>
                    <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "8px", fontWeight: 700, color: "#fff", letterSpacing: "0.04em" }}>Scopri →</span>
                  </div>
                </div>
              </div>
              {/* Feature strip */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", background: "#0A0D12", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                {[["VO₂ Max", "Testing"], ["Calori.", "Indiretta"], ["Valid.", "ISO"]].map(([t, s], i) => (
                  <div key={i} style={{ padding: "9px 8px", borderRight: i < 2 ? "1px solid rgba(255,255,255,0.04)" : "none" }}>
                    <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "8px", fontWeight: 800, color: "#EEF2FF", lineHeight: "1.2", margin: 0 }}>{t}</p>
                    <p style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "6px", fontWeight: 600, color: P.perfBlue, letterSpacing: "0.04em", margin: 0 }}>{s}</p>
                  </div>
                ))}
              </div>
              {/* Bottom tab bar */}
              <div style={{ display: "flex", justifyContent: "space-around", padding: "8px 4px 12px", background: "#0B0F14", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
                {[["🏠","Home"],["🔍","Cerca"],["📋","Clinica"],["👤","Profilo"]].map(([icon, label], i) => (
                  <div key={label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px" }}>
                    <span style={{ fontSize: "13px" }}>{icon}</span>
                    <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "5.5px", fontWeight: 600, color: i === 0 ? P.perfBlue : "rgba(238,242,255,0.4)", letterSpacing: "0.04em" }}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ textAlign: "center", marginTop: "10px" }}>
            <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "7px", fontWeight: 700, color: "rgba(255,255,255,0.32)", letterSpacing: "0.14em", textTransform: "uppercase" }}>Responsive Mobile</span>
          </div>
        </div>
